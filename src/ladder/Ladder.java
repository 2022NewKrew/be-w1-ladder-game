...
public class Ladder {
    ...
    */
    ...
    private void inputLadderHeight() {
        ...
        this.ladderHeight = this.sc.nextInt();
    }

    // 사람의 수를 입력받는 메서드
    private void inputNumOfPersons() {
        System.out.println("참여할 사람은 몇 명인가요?");
        this.numOfPersons = this.sc.nextInt();
    }
    ...
    private void makeLadder() {
        this.isLinked = new boolean[this.ladderHeight][this.numOfPersons - 1];
        for (int i = 0; i < this.ladderHeight; i++) {
            for (int j = 0; j < this.numOfPersons - 1; j++) {
                this.isLinked[i][j] = Math.random() >= 0.5;
            }
        }
    }

    /*
    사다리 연결 정보(isLinked)로 현재 만들어진 사다리의 형태를 출력하는 메서드
    ...
    private void printLadder() {
        ...
        for (int i = 0; i < this.ladderHeight; i++) {
            sb.append("|");
            for (int j = 0; j < this.numOfPersons - 1; j++) {
                sb.append(this.isLinked[i][j] ? "-" : " ").append("|");
            }
            sb.append("\n");
        }
