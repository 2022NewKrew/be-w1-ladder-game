...
public class Ladder {
    private final int START_OF_ROW = 0;
    private final int START_OF_BRIDGE = 1;
    ...
    private List<List<Boolean>> makeLadder(String[] gamePlayers, int ladderHeight) {
        ...
        for(int h = START_OF_ROW; h<this.ladderHeight; h++) { // 한 줄씩 사다리를 생성
    ...
    private List<Boolean> makeRow() { // 한 줄의 사다리 생성
        ...
        for(int w = START_OF_BRIDGE; w<this.gamePlayers.length; w++) {
